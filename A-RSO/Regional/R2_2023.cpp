...
// https://codeforces.com/gym/104155/problem/2
...
ll multiplyFib(ll n, int p) {

    // Two endpoints (when we reach left or right edge, we stop)
    if (n == 1) return 1;
    if (p >= fib.size()) return 0;

    ll res = multiplyFib(n, p + 1);
    if (n % fib[p] == 0) res += multiplyFib(n / fib[p], p);
